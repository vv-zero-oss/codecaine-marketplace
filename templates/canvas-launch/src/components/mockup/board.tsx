...
function FramedProject({ hover }: { hover: boolean }) {
  return (
    <div className="absolute top-12 left-[74px] w-[700px]">
      ...
      <div className="overflow-hidden rounded-[6px] bg-white shadow-ed-frame">
        <div className="flex items-center justify-between border-b border-black/5 px-7 py-4 text-[11px] text-neutral-500">
          <span className="text-[13px] font-semibold tracking-tight text-neutral-900">Tidewater</span>
          <span className="flex gap-5">
            <span>Product</span>
            <span>Pricing</span>
            <span>Customers</span>
          </span>
          <span className="rounded-full border border-black/10 px-3 py-1 text-neutral-800">Sign in</span>
        </div>
        <div className="px-7 pt-10 pb-8">
          <p className="text-[10px] font-semibold tracking-[0.14em] text-ed-accent uppercase">Shift planning</p>
          <p className="mt-2 max-w-[420px] text-[30px] leading-[1.08] font-semibold tracking-[-0.03em] text-neutral-900">
            Plan the week your team will actually keep.
          </p>
          <p className="mt-3 max-w-[380px] text-[12px] leading-[1.55] text-neutral-500">
          ...
          <div className="mt-6 flex items-center gap-4">
            <Selected label="ButtonLink" size="124 × 40" hover={hover}>
              <span className="block rounded-full bg-neutral-900 px-5 py-2.5 text-[12px] font-medium text-white">
                Start free trial
              </span>
            </Selected>
            <span className="text-[12px] font-medium text-neutral-700">See how it works →</span>
          </div>
        </div>
        <div className="grid grid-cols-3 gap-3 px-7 pb-8">
          {["Team", "Business", "Enterprise"].map((plan, i) => (
            <div
              key={plan}
              className={cn(
                "rounded-[10px] border border-black/5 p-4",
                i === 2 ? "bg-neutral-900 text-white" : "bg-neutral-50 text-neutral-900",
              )}
            >
    ...
    <span className="relative inline-block">
      ...
      <span className={cn("pointer-events-none absolute -inset-[3px] rounded-[2px] outline-[1.5px] outline-solid transition-colors", hover ? "outline-ed-state" : "outline-ed-accent")}>
        {["-top-1 -left-1", "-top-1 -right-1", "-bottom-1 -left-1", "-bottom-1 -right-1", "-top-1 left-1/2 -ml-1", "-bottom-1 left-1/2 -ml-1", "top-1/2 -left-1 -mt-1", "top-1/2 -right-1 -mt-1"].map(
          (pos) => (
            <span key={pos} className={cn("absolute size-2 rounded-[2px] border-[1.5px] bg-white shadow-sm", hover ? "border-ed-state" : "border-ed-accent", pos)} />
          ),
        )}
      </span>
      <span className={cn("absolute top-full left-1/2 mt-2.5 -translate-x-1/2 rounded-[3px] px-1.5 py-0.5 text-[10px] font-medium whitespace-nowrap text-white", hover ? "bg-ed-state" : "bg-ed-accent")}>
    ...
    <div className="absolute bottom-4 left-1/2 flex h-11 -translate-x-1/2 items-center gap-1.5 rounded-full bg-ed-bar px-2 shadow-ed-bar">
      {slots.map(({ Icon, active, menu }, i) => (
        <span key={i} className="flex items-center">
          <span className={cn("grid size-7 place-items-center rounded-full text-white", active && "bg-ed-bar-accent")}>
            <Icon className="size-[17px]" strokeWidth={1.5} />
          </span>
          {menu && <ChevronDown className="w-3.5 text-white/70" size={10} strokeWidth={2} />}
        </span>
      ))}
      ...
      <span className="flex h-8 items-center rounded-full bg-ed-bar-line p-0.5">
        ...
        <span className="grid size-7 place-items-center text-white/70">
