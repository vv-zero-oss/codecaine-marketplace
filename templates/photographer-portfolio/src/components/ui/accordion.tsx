...
function AccordionItem({
  ...
  return (
    <AccordionPrimitive.Item className={cn("border-b border-stone-200", className)} {...props} />
  )
}
...
function AccordionTrigger({
  ...
  return (
    <AccordionPrimitive.Header className="flex">
      ...
      >
        {children}
        <ChevronDown className="size-4 shrink-0 text-stone-400 transition-transform duration-200" />
      </AccordionPrimitive.Trigger>
...
function AccordionContent({
  ...
  return (
    <AccordionPrimitive.Content
      ...
      {...props}
    >
      <div className={cn("pb-5 leading-relaxed text-stone-600", className)}>{children}</div>
    </AccordionPrimitive.Content>
  )
}
